...
public class Game {
                ...
                if (player.hasCard(card)) {
                    if (card.canPlay(this)) {
                        ...
                        if (card.getType() == Card.WDF)
                            ...
                            if (player.hasUno() && player.isABot)
    ...
    public void join(String name, boolean bot) {
        Card[] cards = new Card[7];
        for (int i = 0; i < 7; i++) {
            cards[i] = draw.drawCard();
        }
    ...
    public void nextPlayer() {
        ...
        hasDrawn = false;
        if (currentPlayer.isABot) {
            bot.play(currentPlayer, players, draw, this);
            //nextPlayer();
        }
    }
