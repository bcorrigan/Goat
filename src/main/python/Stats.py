...
WORD_TYPES_RX = {
    ...
    RACISM_COUNT: (True, re.compile(
        r'(kike|chink|nigger|\bspick?\b|gook|boche|wetback|\bkraut\b|honkey|porch\s*monkey|raghead|camel\s*jockey|darkie|greaser|paddy|paddie|\bmick\b|pikey|fenian|gypsy|shylock|m[ou]sselman|moslem|gringo|porridge\s*wog|white\s*(pride|power)|slit[a-z]*\s*eye|red\s*indian|juden|dago|paki\b|haj|anglos?(\s+|$)|whitey|negro|beaner|confucius\s*say)'
    )),
    ...
    SEX_COUNT: (False, re.compile(
        ...
        r'(sex|fuck|cock|butt|puss|vag|cunt|slit|kitty|schlong|penis|\bballs?\b|pecker|member|muff|glans|head|blow|suck|lick|swallow|spit|deep|throat|tight|loose|hard|soft|tit|breast|\bnip|knocker|cup|double\s*d|porn|virgin|chaste|pure|impure|bondage|discipline|sado|sadism|masochis|punish|spank|tie|69|plow|rape|ream|bugger|ride|missionary|dog[a-z]*\s*style|cowgirl|wheelbarrow|sodom|anal|ero[ts]|lesb|insert|slam|complet|fetish|fan\s*fic|fantasy|libid|pound|master|slave|subjugat|penetrat|thrust|us(e|ing)|cum|jiz|jism|semen|ejac|jack|get\s*(off|some)|enter|moan|orgasm|deflower|ravage|ravish|violate|defile|maiden|celibate|vestal|foreplay|caress|cuddl|pet|kiss|love|mak[a-z]*\s*out|oral|fornicat|coitus|copulat|intima|relation|sleep[a-z]*\s*(around|with)|screw|carnal|nooky|intercourse|coupl|consumm|mate|mating|shove\s*it|up\s*your|mom|mother|dad|father|oedip|pa?edo|epheb|furr(y|ies)|yiff|ass|boot(y|ie)|junk|camel\s*toe|yoga\s*pants|dress|skirt|pant(y|ies)|\bbra\b|lingerie|negligee|condom|birth\s*control|the\s*pill|depo|estrogen|testosterone|yaz|facial|cream|double\s*pen|teen|fap|beast.*two\s*backs|strap-?\s*on|dildo|finger|reproduc|cunnilingus|std|hiv|aids|incest|minor|prostitut|whore|pimp|slut|bestiality|pregna|pleasure|gonorrhea|clap|chlamydia|syphilis|\bhep\b|hepatitis|herpes|hpv|crab|anilingus|toss[a-z]*\s*salad|vulva|trib|labia|masturbat|scissor|frot|dagger|hump|scrump|grind|rub|job|phall(us|ic)|venus|phil(ia|e)|dental\s*dam|latex|silicon|implant|enlarge|engorge|flaccid|turgid|passion|torrid|affair|extramarital|sultr|ardent|spurt|spray|shoot|throb|nut|huevo|fertil|egg|cervix|uter(us|ine)|fallopian|ovar(y|ies)|nail|(take|get)\s*it|cross|tran(n|s)|gender|figure|bdsm|split|divid|cleave|cigar|banana|gun|pistol|ammunition|ammo|tower|sausage|wiener|hot\s*dog|pipe|tube|wank|jerk|drill|inside|tast[ei]|tongue|\blips?\b|fag|ga+y+|fudge|chocolate|pack|tuck|sodom|bugger|clit|force|\brams?\b|shag|beat|meat|pok(e|ing)|\bdo\b|long|firm|backside|thick|sweat|three-?\s*some|menage|\bin\b|stick|forbid|fruit|peach|grape|man|men|child|girl|boy|rear|behind|solid|rifle|\btop|\bout\b|lub(e|ric)|\bwet\b|satisf|boob|mount|bottom|twink|\bbear|size|viagra|cialis)'
    ))
}
...
class Stats(Module):
    ...
    def purity_fail(self, m, store, is_channel):
        ...
        # temporary workaround here to expunge some bad data
        best_scorer = store.getOrElse(PURITY_BEST_FAILED_SENDER, None)
        if best_scorer == "tester3":
            best = 0
