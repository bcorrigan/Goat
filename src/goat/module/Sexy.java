...
public class Sexy extends Module {
    ...
    private long getNumResults(String searchString) throws IOException {
        ...
        try {
            URL url = new URL("http://" + google + "/search?q=" + searchString.replaceAll(" ", "+"));
            ...
            if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
                throw new IOException("Hmmmn, " + google + " is giving me an HTTP Status-Code " + connection.getResponseCode() + ", sorry.");
            } else {
					// Debuggo
					//System.out.println("Sexy: connection OK") ;
				}
            in = new BufferedReader(new InputStreamReader(connection.getInputStream()));
				// Debug block
				in.mark(2048) ;
				if (null == in.readLine()) {
					System.out.println("Sexy: Recieved null content from " + google) ;
				} else {
					in.reset() ;
				}
            ...
            while ((inputLine = in.readLine()) != null) {
                if (inputLine.matches(".*color=>Results.*")) { //in the content line
						  // Debuggo 
						  // System.out.println("Sexy: parsing line: " + inputLine) ;
                    ...
                    inputLine = inputLine.replaceAll(",", "");         //get rid of commas, should just be left with a number now
						  // Debuggo
						  // System.out.println("Sexy: left with: " + inputLine) ;
                    try {
                        return Long.parseLong(inputLine);
                    } catch (NumberFormatException nfe) {
                        nfe.printStackTrace();
                        throw new IOException("Sexy: omg a parse error: " + nfe.getMessage());
                    }
                } else {
						 // Mondo Debuggo
						 // System.out.println("Sexy: " + inputLine) ;
					 }
            }
        } finally {
            if (in != null) in.close();
